...
generated by content, and writes a bounded JSON report. When the shared
snapshot backend is not enabled, the same command composes each manifest route
through the synchronous BFF delivery seam; it does not maintain a second list
of page dependencies or a legacy Web-side resolver. `start-dev.sh` starts the
applications but does not run this command automatically; invoke it after the
stack is ready when a cold local cache is part of the test.
